...
class Verifier:
    ...
    def add_relu_layer(self, z, lower_bounds, upper_bounds):
        ...
        # For LP formulation: use continuous variables (0, 1) instead of binary
        p_vars = self.gurobi_model.addVars(n_inputs, lb=0.0, ub=1.0)  
        self.gurobi_model.update()
...
def verify(model, input_image, groundtruth_label, perturbation):
    ...
    for (i, r) in enumerate(target_labels):
        ...
        # Since LP only provides lower bound
